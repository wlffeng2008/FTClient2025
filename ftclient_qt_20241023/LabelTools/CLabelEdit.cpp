...
std::shared_ptr<WgtLabelParaStart> CLabelEdit::getWgtStart()
{
    if( nullptr == m_pWgtStart )
    {
        ...
        connect( m_pWgtStart.get(), &WgtLabelParaStart::sigPara,this, [=]( label::LabelPara p ){
            m_para = p;
            updateWinSize();
            this->show();
        } );
        connect( m_pWgtStart.get(), &WgtLabelParaStart::sigStartEdit, this, [=](){
            this->show();
        } );
    }
    return m_pWgtStart;
...
bool CLabelEdit::findItemByTypeIn( QGraphicsItem **ppItem, const QString& typeIn,int nIndex)
{
    ...
    for( auto item : getScene()->items() )
    {
        ...
        {
            QString type_in = pixmapItem->data(0).toString();
            if( typeIn == type_in )
            {
                *ppItem = pixmapItem;
                nFind++;
        ...
        {
            QString type_in = textItem->data(0).toString();
            if( typeIn == type_in )
            {
                *ppItem = textItem;
                nFind++;
...
void CLabelEdit::initToolBar()
{
    ...
    buttonAction = new QAction( "添加横线", this );
    toolBar->addAction( buttonAction );
    connect( buttonAction, &QAction::triggered, this, &CLabelEdit::onAddHLine );

    buttonAction = new QAction( "添加竖线", this );
    toolBar->addAction( buttonAction );
    connect( buttonAction, &QAction::triggered, this, &CLabelEdit::onAddVLine );
    toolBar->addSeparator() ;

    buttonAction = new QAction( "添加条码SN", this );
    toolBar->addAction( buttonAction );
    ...
    buttonAction = new QAction( "模板参数设置...", this );
    toolBar->addAction( buttonAction );
    ...
    m_pLineEdit_x = new QLineEdit(this);
    connect( m_pLineEdit_x, &QLineEdit::editingFinished, this,[this](){
        this->on_label_rect_editingFinished();
    } );
    toolBar_sub->addWidget( label );
    toolBar_sub->addWidget( m_pLineEdit_x);

    label = new QLabel(" Y: ");
    m_pLineEdit_y = new QLineEdit(this);
    connect( m_pLineEdit_y, &QLineEdit::editingFinished, this,[this](){
        this->on_label_rect_editingFinished();
    } );
    toolBar_sub->addWidget(label);
    toolBar_sub->addWidget( m_pLineEdit_y);

    label = new QLabel(" W: ");
    m_pLineEdit_w = new QLineEdit(this);
    connect( m_pLineEdit_w, &QLineEdit::editingFinished, this,[this](){
        this->on_label_rect_editingFinished();
    } );
    toolBar_sub->addWidget( label );
    toolBar_sub->addWidget( m_pLineEdit_w);

    label = new QLabel(" H: ");
    m_pLineEdit_h = new QLineEdit(this);
    connect( m_pLineEdit_h, &QLineEdit::editingFinished, this,[this](){
        this->on_label_rect_editingFinished();
    } );
    toolBar_sub->addWidget( label );
    toolBar_sub->addWidget( m_pLineEdit_h );

    m_pLineEdit_x->setMaximumWidth(60) ;
    m_pLineEdit_y->setMaximumWidth(60) ;
    m_pLineEdit_w->setMaximumWidth(60) ;
    m_pLineEdit_h->setMaximumWidth(60) ;

    toolBar_sub->addSeparator() ;

    QPushButton *pFontBtn = new QPushButton("字体设置...",this);
    toolBar_sub->addWidget( pFontBtn );
    pFontBtn->setMinimumWidth(100) ;
    connect(pFontBtn,&QPushButton::clicked,this,[this]()
    {
    ...
    {
        QPushButton *buttonAction = new QPushButton( "打 印", this );
        toolBar_sub->addWidget( buttonAction );
        buttonAction->setFixedWidth(80) ;
        connect( buttonAction, &QPushButton::clicked, this, &CLabelEdit::onPrint );
        toolBar_sub->addSeparator() ;

        buttonAction = new QPushButton( "打印预览", this );
        toolBar_sub->addWidget( buttonAction );
        buttonAction->setFixedWidth(80) ;
        connect( buttonAction, &QPushButton::clicked, this, &CLabelEdit::onPrintPreview );
        toolBar_sub->addSeparator() ;

        m_pAutoPrt = new QCheckBox( "自动打印", this );
        toolBar_sub->addWidget( m_pAutoPrt );
    }
}
...
void CLabelEdit::setMode(int nMode)
{
    ...
    pSet->getPrintMatrix(nMode,nRows,nCols) ;
    ...
    if(nMode == 3)
    {
        ...
        for(int i=0; i<2; i++)
        {
            for(int j=0; j<10; j++)
            {
                ...
                textItem->setData( 0, label::sTypeInBarSN );
                textItem->setFont(s_font) ;
                textItem->setScale(1.2);

                connect( textItem, &CustomTextItem::sigRectChanged_ , this, [this]( QRectF rect ){
                ...
                pixmapItem->setData( 0, label::sTypeInBarCode );
                pixmapItem->setScale(1) ;

                connect( pixmapItem, &CustomPixmapItem::sigRectChanged_ , this, [this]( QRectF rect ){
                    this->onCurItemRectChanged( rect );
                } );
            }
        }
...
void CLabelEdit::setLabelInfo(const QString&strUuid,const QString&strSN)
{
    ...
    if( findItemByTypeIn( &pItemFound, label::sTypeInQrCode ) )
    {
        ...
        QImage img = genQrCode(strUuid.toStdString()) ;//strQR.toStdString()
    ...
    if( findItemByTypeIn( &pItemFound, label::sTypeInBarSN ))
    {
        ...
        barSNItem->setPlainText("S/N: " + strUuid) ;
    }

    if( findItemByTypeIn( &pItemFound, label::sTypeInBarSN,1 ))
    {
        //CustomTextItem *barSNItem = dynamic_cast<CustomTextItem*>(pItemFound) ;
...
void CLabelEdit::updateWinSize()
{
    ...
    getScene()->setSceneRect( 0, 0, w_win-4, h_win - 60 );
    getView()->setFixedSize( getScene()->sceneRect().size().toSize() );
}
...
void CLabelEdit::onAddImg()
{
    ...
    if( !fileName.isEmpty() )
    {
        ...
        pixmapItem->setData(0, label::sTypeInImage );
...
void CLabelEdit::onAddHLine()
{
    QPixmap A( width(),5 ) ;
    A.fill(Qt::black) ;

    CustomPixmapItem* pixmapItem = new CustomPixmapItem( A );
    getScene()->addItem( pixmapItem );
...
void CLabelEdit::onAddVLine()
{
    ...
    CustomPixmapItem* pixmapItem = new CustomPixmapItem( A );
    getScene()->addItem( pixmapItem );
    pixmapItem->setPos( 10, 0 );
    pixmapItem->setData(0, label::sTypeInImage );

    connect( pixmapItem, &CustomPixmapItem::sigRectChanged_ , [this]( QRectF rect ){
        this->onCurItemRectChanged( rect );
    } );
}
...
void CLabelEdit::onAddText()
{
    ...
    getScene()->addItem( textItem );
    textItem->setPos( 0, 0 );
    textItem->setData(0, label::sTypeInText );
    textItem->setFont(s_font) ;

    connect( textItem, &CustomTextItem::sigRectChanged_ , [this]( QRectF rect ){
        this->onCurItemRectChanged( rect );
    } );
}
...
void CLabelEdit::onAddQrCode()
{
    ...
    pixmapItem->setPos( width()*0.3, height()*0.3 );
    pixmapItem->setData(0, label::sTypeInQrCode );
