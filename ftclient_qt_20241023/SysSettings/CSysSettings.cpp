...
{
    m_sCurDeviceComType = curDeviceComType.toStdString();
    return true;
}
...
void CSysSettings::setPrintMatrix(int nType,int nRows,int nCols)
{
    switch(nType)
    {
    case 0:
        m_jsonLabelTemCfg["devprintrow"]=nRows;
        m_jsonLabelTemCfg["devprintcol"]=nCols;
        break;
    case 1:
        m_jsonLabelTemCfg["boxprintrow"]=nRows;
        m_jsonLabelTemCfg["boxprintcol"]=nCols;
        break;
    case 2:
        m_jsonLabelTemCfg["packprintrow"]=nRows;
        m_jsonLabelTemCfg["packprintcol"]=nCols;
        break;
    }
}

void CSysSettings::getPrintMatrix(int nType,int &nRows,int &nCols)
{
    nRows = 1 ;
    nCols = 1 ;

    if(!m_jsonLabelTemCfg.contains( "devprintrow" ))
        return ;

    switch(nType)
    {
    case 0:
        nRows = m_jsonLabelTemCfg["devprintrow"].toInt();
        nCols = m_jsonLabelTemCfg["devprintcol"].toInt();
        break;
    case 1:
        nRows = m_jsonLabelTemCfg["boxprintrow"].toInt();
        nCols = m_jsonLabelTemCfg["boxprintcol"].toInt();
        break;
    case 2:
        nRows = m_jsonLabelTemCfg["packprintrow"].toInt();
        nCols = m_jsonLabelTemCfg["packprintcol"].toInt();
        break;
    }
}

bool CSysSettings::getHttpPara( std::string &sIP, int &iPort )
{
    sIP = "120.79.120.31";
    iPort =  2375;
    if(m_jsonNetCfg.contains( "http_server_ip" ) )
    {
        sIP = m_jsonNetCfg["http_server_ip"].toString().trimmed().toStdString();
        iPort = m_jsonNetCfg["http_server_port"].toInt();
    }
    return true;
}
...
{
    qDebug() << "setCurDeviceType" << curDeviceType;
    m_sCurDeviceType = curDeviceType.toStdString();
    return true;
}
...
bool CSysSettings::loadDevTypeSetDefaultCmds( const std::string &devType, std::vector<QJsonObject> &vecItems )
{
    ...
    while( std::getline( file, line ) ) {
        ...
        QJsonObject cmdJson;
        cmdJson["var_group"] = strList[0].trimmed();
        cmdJson["var_name"] = strList[1].trimmed();
        cmdJson["var_value"] = strList[2].trimmed();
        cmdJson["var_data_type"] = strList[3].trimmed();
        cmdJson["var_note"] = strList[4].trimmed();
