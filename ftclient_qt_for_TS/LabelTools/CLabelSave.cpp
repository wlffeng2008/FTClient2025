...
bool CLabelSave::saveSceneWithImages(QGraphicsScene *scene, const QString &filePath,TemplInfo&Info)
{
    ...
    for (auto item : scene->items())// 遍历场景中的所有对象
    {
        if (auto textItem = dynamic_cast<CustomTextItem*>(item))
        {
            ...
            textObject["type_in"] = textItem->data( label::obj_type_in_data_index ).toString();
        ...
        else if (auto pixmapItem = dynamic_cast<CustomPixmapItem*>(item))
        {
            ...
            pixmapObject["type_in"] = pixmapItem->data( label::obj_type_in_data_index ).toString();

            // 将图片转换为 QImage，然后编码为 Base64
...
bool CLabelSave::loadSceneWithImages(QGraphicsScene *scene, const QString &filePath,TemplInfo&Info)
{
    ...
    qDebug()<<"loadSceneWithImages:" << filePath ;
    ...
    for( const QJsonValue &value : itemsArray )
    {
        ...
        if (type == "text")
        {
            ...
            textItem->setData( label::obj_type_in_data_index, type_in );
        ...
        else if (type == "pixmap")
        {
            ...
            pixmapItem->setData( label::obj_type_in_data_index, type_in );
