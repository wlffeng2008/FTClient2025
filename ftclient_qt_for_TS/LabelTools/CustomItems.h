...
public:
    ...
    void emitRectSig()
    {
        ...
        //qDebug() << "mouse move event signal sent ok: " << getItemCurRect();
    }
    ...
    void handleMousePressEvent( QGraphicsSceneMouseEvent *event )
    {
        ...
        //qDebug() << "event scenePos: " << event->scenePos() << ", scenePos: " << m_item->scenePos() << ", itemPos: " << m_item->pos() << ", boudingRect: " << rect;
    ...
    void setItemRect( const QRectF rectNew )
    {
        if( m_item != nullptr )
        {
            ...
            qDebug() << "New rect: " << getItemCurRect();
...
protected:
    ...
    void paint( QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget ) override {
        ...
        if(this->isSelected())
        {
            ...
            QPen pen = QPen(QColor(qRgba(255,0,0,200))) ;
            pen.setWidth(1) ;
            painter->setPen(pen) ;
...
public:
    ...
    {
        bool b = QObject::connect( &base, &CustomBaseItem::sigRectChanged, [=]( QRectF rect ){
            emit sigRectChanged_( rect );
        } );
    }
