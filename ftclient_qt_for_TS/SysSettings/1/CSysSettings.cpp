...
QString CSysSettings::getOrderId()
{
    return m_strOrderId;
}

void  CSysSettings::setOrderId(const QString&strOrderId)
{
...
bool CSysSettings::getHttpPara( std::string &strIP, int &iPort )
{
...
bool CSysSettings::setRtmpPara( const std::string &strIP, int iPort )
{
    m_jsonNetCfg["rtmp_server_ip"] = strIP.data();
    m_jsonNetCfg["rtmp_server_port"] = iPort;
    return true;
}

bool CSysSettings::getRtmpPara( std::string& strIP, int& iPort )
{
    strIP = m_jsonNetCfg["rtmp_server_ip"].toString().toStdString();
    iPort = m_jsonNetCfg["rtmp_server_port"].toInt();
    return true;
}

bool CSysSettings::setRtmpDelay_ms( int iDelay_ms )
{
    m_jsonNetCfg["rtmp_delay_ms"] = iDelay_ms;
    return true;
}
...
bool CSysSettings::loadSysConfig()
{
    ...
    if( jsonAllData["OrderId"].isString() )
        m_strOrderId = jsonAllData["OrderId"].toString();
...
bool CSysSettings::saveSysConfig()
{
    ...
    qDebug() << "cur label tem config: " << m_jsonLabelTemCfg;
...
bool CSysSettings::loadDevTypeTestCmds( const std::string &devType, std::map<int,QJsonObject>& mapTestItems )
{
    ...
    while( std::getline( file, line ) ) {
        // 第一行是文件头，用于注释数据列的意义, 每一列意义如下：
        // #指令名称, #指令内容, #是否需要手动pass, #指令写入参数, #返回核对参数(返回值与该值比对一致为ok), #指令执行超时时间(s)，#按钮文字
        // 内容为 null 字符串的，意为空
        // 超时时间为0，表示采用系统默认值
        if( -1 == index )
        {
        ...
        // 提取item_name和item_cmd
        lineList = QString::fromStdString( line ).split( "," );
...
bool CSysSettings::loadDevTypeSetDefaultCmds( const std::string &devType, std::vector<QJsonObject> &vecItems )
{
    ...
    std::string cfgFilePath = m_mapDevTypeSettings[devType]["default_var_value_file"].toString().toStdString(); // "D:/Work/Projs/FactoryTest/FTClient/FTClient_qt/TS201Q.set_default";       // ?? 待补充
    std::ifstream file( cfgFilePath );
    if( !file.is_open() )
    {
        qDebug() << "Failed to open file: " << cfgFilePath.data() ;
    ...
    while( std::getline( file, line ) ) {
        ...
        cmdJson["var_data_type"] = strList[2].trimmed();
